...
<Callout type="tip" title="Tip">
  The system is designed to work with partial data. Even if some sources fail or data is incomplete, you'll still get a functional books page with whatever data is available.
</Callout>

<Callout type="warning" title="Warning">
  Don't commit cookies or API keys to version control. Use environment variables and `.gitignore` to keep sensitive data secure.
</Callout>

<Callout type="note" title="Note">
  The system runs automatically on every build (`npm run build`), so your books page stays up-to-date without manual intervention.
</Callout>
